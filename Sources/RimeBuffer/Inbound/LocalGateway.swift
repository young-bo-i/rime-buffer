    ...
    private final class Connection {
        ...
        private func parse(_ data: Data) -> (Req, Int)? {
            guard let end = data.range(of: Data("\r\n\r\n".utf8)) else {
                return data.count > LocalGateway.maxHeaderBytes ? nil : nil
            }
        ...
        // Minimal MCP streamable-HTTP (validated against real Claude Code).
        private func handleMCP(_ req: Req) {
            ...
            switch method {
            case "initialize":
                mcpSessionID = "s-" + UUID().uuidString.prefix(8)
                if let ci = (msg["params"] as? [String: Any])?["clientInfo"] as? [String: Any],
                   let name = ci["name"] as? String { mcpClientName = name }
                json(["jsonrpc": "2.0", "id": id, "result": [
                    "protocolVersion": "2024-11-05",
                    "capabilities": ["tools": [String: Any]()],
                    "serverInfo": ["name": "Enter输入法", "version": "1"],
