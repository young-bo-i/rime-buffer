...
10. **意识流 raw 不是输入法 preedit**：只有缓冲开启 + 唯一 owner + 非 secure + 精确外部焦点同时成立时，物理 `a-z` 才在 Rime 前进入 `StreamInputWorkspace`。workspace 始终把它解释为连续全拼，但不读取、切换或持久化当前输入方案；raw 严格只含小写 ASCII 字母，Space/标点消费但不写入，数字只控制候选。它不进入 BufferModel、候选、遥测正文或宿主。字母只重置 220 ms debounce，800 ms burst 上限不重置；最多两路 make-before-break，旧结果不进入新 prompt，迟到回调按 job/generation 作废。每轮 prompt 包含完整 raw 与最多三条 lossless 本地音节提示。partial 使用稳定候选槽位，final 精确覆盖，旧尾不能进入 ready/交付。输出是 1–3 个完整互斥猜测；首块投递后候选锁定，继续输入会撤销未发尾部并建立全新 raw，已经发送的前缀不得复活。
...
- **意识流调度与连续渲染**：连续字母只重置 220 ms debounce；本次 burst 首键建立的 800 ms max-wait 不重置，因此不停打字也会按上限刷新。Space 与标点只消费，不修改 raw/revision，也不触发请求；数字 `1`–`3` 只切换尚未开始投递的候选。workspace 采用有界 make-before-break，最多允许旧视觉 producer + 新 challenger 两路；旧响应和 baseline 只服务于显示，绝不进入下一轮 prompt，墓碑回调按 job/generation 丢弃。
- **意识流全局推断边界**：请求边界始终冻结并发送当前完整纯字母 raw 拼音，任何响应都是对全文的替代解释。`StreamInputPinyinHints` 产生最多三条 lossless 边界提示；未知 English/错键不丢失，超过 512 bytes 时省略提示。候选 index 是稳定槽位，final 逐槽精确替换 partial。首块投递后锁定候选；继续输入会建立全新 raw 并撤销未发送尾部，已投递前缀不得复活。
