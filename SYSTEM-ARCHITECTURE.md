...
RIMES 是一个 **macOS 中文输入法**（IMKit + 自包含 librime），并在其上叠加了一个**独立、常驻、上屏前的文本工作台**：本地打字、已接受的外部文字与用户主动请求的插件结果汇入缓冲区，经人工确认后投递到实时校验的输入框。普通工作台折叠/展开为 44/78pt；苹果本地翻译、「AI 生成」或「意识流输入」打开时改为上 source/下 target 两条图标标识的缓冲轨，折叠/展开为 78/112pt，拖拽/展开对齐源文行，右侧主控件对齐目标行。「AI 生成」右侧主按钮在禁用 AI、可请求 AI、转圈等待和纸飞机投递间原位切换，展开层不再放独立生成按钮；Return 与主按钮共用同一 AI 状态机。「AI 生成」与 Action Plugin 使用“连接器 › AI 模型”中独立选择的 Codex CLI、Claude Code CLI 或 OpenAI 兼容 API；「意识流输入」不跟随该单选，固定读取用户保存的 OpenAI 兼容配置（当前模型 `deepseek-v4-flash`）。上层含状态、缓冲插件选择器与其他插件的当前动作、刷新/重置和关闭；选择器可原地切换全部 `.bufferAction` 插件的唯一 owner。普通 AI 只在用户用 Return 或右侧主按钮明确请求时运行；意识流在满足缓冲/全拼串击/精确焦点门时按输入停顿自动猜测，并在确有歧义时给出最多三个完整、互斥版本。用户用数字 `1`–`3` 切换选择，Return 或纸飞机只发送所选版本；任何结果都不能自动上屏。刷新/重置保留普通派生源正文；关闭、secure input 或焦点失效会清空意识流的焦点绑定 raw。`Command+Shift+B` 可从任意应用全局切换工作台：关闭时打开并恢复捕获，打开时按普通关闭语义收束组字、保留内容并暂停捕获。工作台本身不提供块编辑器或缓冲开关；常规候选与 preedit 继续由 `CandidateWindow` 呈现，意识流 raw 从不进入二者。
...
7. **缓冲按键与宿主隔离**：缓冲模式下普通/Shift+Return 与 Backspace 总是被输入法消费。有未决 Rime/并击/raw 输入时，本次 Return 只收束成缓冲块并抑制同一物理按键余下事件；没有未决组字时，Return 在 keyDown 中定点重建不可见 marked-text guard。普通/ready 内容仍是轻按发送下一块、按住约 1.2 秒发送全部；AI request 状态则在 keyDown 就吞下整次物理按键并请求生成，running/disabled 只吞键，不进入长按计时。`didCommand` 与 repeat 只有消费权。Backspace 只在精确焦点下编辑 Rime/并击状态或删除缓冲块。焦点不可信时始终吞键且不投递；宿主绝不会收到换行或删除。
8. **派生双轨按生成快照交易**：翻译与普通 AI 只有已完成且仍匹配 source text/block ids/generation 的 target blocks 可投递；目标块未全部送完时源块原样保留，最后一个目标块成功消费后才一次性消费对应源块。意识流的 1–3 个 target 是互斥备选而非待发送队列，投递快照只含所选项；该项成功后才一起清除本轮 raw 与其他备选。
...
10. **意识流 raw 不是输入法 preedit**：只有缓冲开启 + 全拼串击 + 唯一 owner + 非 secure + 精确外部焦点同时成立时，plain `a-z` 与其拥有的分隔键才在 Rime 前进入 `StreamInputWorkspace`；它不进入 BufferModel、候选、遥测正文或宿主。字母只重置 220 ms debounce，800 ms burst 上限不重置；Space 规范化为一个短句硬边界并立即触发当前完整 raw，首部/连续 Space 去重，后续输入仍有停手后的尾部刷新，其他标点继续消费并忽略。workspace 采用最多两路的 make-before-break：旧路只维持视觉 baseline，新 challenger 首个合法 snapshot/终态后才取消旧路；两路占满只覆盖一个 latest-only pending。raw 一变，所有旧请求立即失去投递资格，仍可产出无选中态、不可提交的临时 partial/final。旧显示/结果不进入新 prompt，墓碑后的迟到回调按 job/generation 作废。每轮 prompt 仍包含完整 raw，并可附最多三条 lossless 本地音节边界提示；提示保留 Space 短句边界，不生成中文、不触发分区模型调用，超过 512 bytes 时直接省略但完整 raw 仍发送。新 partial 按公共前缀与稳定候选槽位原位更新，短分歧达到可读长度后才替换，final 必须精确覆盖本轮结果，任何旧尾都不能进入 ready 状态或交付。异步结果和投递接口还必须再次匹配 raw、冻结焦点与 generation，并绑定同一份用户保存的 OpenAI 兼容配置。输出是 1–3 个完整、互斥猜测，数字只改变选择，投递永远只包含当前选中项。
...
- **意识流调度与连续渲染**：连续字母只重置 220 ms debounce；本次 burst 首键建立的 800 ms max-wait 不重置，因此不停打字也会按上限刷新。Space 规范化为短句边界并立即进入同一调度，后续字母停手仍请求最新完整 raw；首部/重复 Space 去重，其他标点消费但忽略。workspace 采用有界 make-before-break，最多允许旧视觉 producer + 新 challenger 两路；旧路不会在每个按键时取消，可继续产出临时 partial/final，但 raw 变化已撤销它的 result/delivery lease，waiting/running 阶段也不渲染选中态。新路首个合法 snapshot 或终态才取消旧路；两路占满时所有新边界合并为一个 latest-only pending，槽位打开后捕获当时最新完整 raw。旧响应和 baseline 只服务于显示，绝不进入下一轮 prompt，墓碑回调按 job/generation 丢弃。
- **意识流全局推断边界**：请求边界始终冻结并发送当前完整 raw 拼音，任何响应都是对全文的替代解释，不是对新增后缀的增量翻译。`StreamInputPinyinHints` 以 24 宽 beam 和无声调音节表产生最多三条 lossless 边界提示；Space 不跨越，未知 English/错键不丢失，超过 512 bytes 时省略提示但完整 raw 仍照常发送。它不生成中文也不拆分模型请求。候选 index 是稳定槽位；partial 在同槽内按公共前缀续写，开头分歧少于四个可读字符时先保持旧 baseline，达到阈值后才替换。当前完整 raw 的 final 逐槽精确替换本轮 partial 并决定 ready 内容，禁止把不同 generation、不同 raw 分区或 baseline 旧尾拼成可交付字符串。
- **共同隐私/体验边界**：普通「AI 生成」只在用户用 Return 或右侧 AI 主按钮明确请求时运行，prepared Action Plugin 只在用户点击动作时运行；意识流是唯一按输入停顿自动请求的 AI 例外，且只发送当前焦点绑定的 raw 全拼。普通「AI 生成」只发送当前 `BufferModel.stagedText`，不附带历史、preedit、剪贴板或屏幕上下文；prepared Action Plugin 只发送插件明确返回且通过身份校验的 prompt。首字前仅展示安全的连接/思考摘要/重试/校验状态与等待秒数，不展示 raw chain-of-thought。普通生成正文可细分为受保护的语义 block；意识流的每个结果则必须是可独立投递的完整猜测，不得在候选间拆句。未 review 的 Action Plugin 目标绑定块不能被作为普通 AI 源文。
...
**测试**：无 XCTest target；CI 运行编进二进制的 smoke 子命令。`schema-smoke` 覆盖编码/键入映射与旧偏好迁移，`plugin-platform-smoke` 覆盖缓冲插件单选，`translation-smoke` 覆盖 latest-wins、debounce/max-wait、译文专用投递、部分失败保留与 remote echo 继承。`stream-input-smoke` 覆盖全拼/缓冲/焦点/secure/modifier 捕获门、仅 debounce 重置而 burst max-wait 保持、Space 短句边界立即请求/首部与重复去重/继续输入后的全文尾部请求、其他分隔键消费且忽略、慢首 token 下旧+新最多两路、第三边界 latest-only 合并、首个新 snapshot 交接与迟到回调 tombstone、每轮完整 raw 与全局重算边界、lossless 音节提示/切分歧义/English span/超长省略、旧结果不可投递 baseline、短分歧阈值/公共前缀/稳定槽位连续渲染、waiting/running 无选中态、final 精确覆盖且不残留旧尾、OpenAI 兼容专用路由、1–3 个完整互斥猜测、数字选择及只投递选中项。`ai-text-smoke` 用假 runner/provider、真实短生命周期子进程与本地 URL fixture 覆盖单一 workspace、三连接器切换、prepared prompt、CLI 小增量在 EOF 前到达、Codex app-server 协议/MCP-empty 门、Codex 登录域名/loginId/account-read 状态机、拆分/合并 JSONL、登录 success/cancel/timeout/真实退出 exactly-once、Claude stream-json、SSE `[DONE]` 不等 EOF、cancel/DONE exactly-once、大小写 JSON fence、中文 SSE 任意字节切分、意识流 non-thinking/JSON/max-token 请求体、活动状态、细粒度且保护 URL/数字/引文/代码的 block、独立 Codex home、0600 配置及升级保留、源快照作废、完整 target 投递后才消费 source 与 Action Plugin 源洗权拒绝；这些 smoke 不调用真实模型或用户配置的真实 API。真实 librime 词库桥另有强制隔离 `RIMEBUFFER_USER_DIR` 的 `user-lexicon-bridge-smoke`，不触碰 live 用户库。
