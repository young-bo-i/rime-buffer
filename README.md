...
Codex/Claude 连接器在本机直接启动已登录 CLI，使用各自的订阅登录态，而不是由 Marine 保存 API Key；“本机启动”也不等于本地推理，正文或 Marine 准备好的话术仍会经所选 CLI 的服务发送。Codex 使用 app-server 的 answer delta，并把专用 ChatGPT 登录保存在 `~/Library/RimeBuffer/ai/codex-home`，不会继承用户 `~/.codex` 里的 MCP、Hook、插件或技能；未授权时可直接在“设置 › 连接器 › AI 模型”的 Codex 卡片打开浏览器登录，等待、取消与重新授权都有明确状态。发送正文前还会再次断言 MCP 列表为空。Claude 使用 `stream-json` partial。RimeBuffer 固定命令参数、关闭工具调用与会话持久化、隔离临时工作目录并对 CLI 版本做 fail-closed 兼容检查。OpenAI 兼容连接器要求服务提供 SSE，在同一设置页配置 Base URL、model 和 API key；密钥与配置保存到 `~/Library/RimeBuffer/ai/openai-compatible.json`，文件权限为 0600，不写入 UserDefaults 或日志。
