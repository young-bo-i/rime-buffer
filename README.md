...
「AI 生成」现在是唯一的内置 AI 缓冲插件；原来的「Codex CLI」「Claude Code CLI」「通用 Open API（OpenAI 兼容）」不再分别占用三个插件位，而是在“设置 › 连接器 › AI 模型”中作为三个可切换的模型源。插件 owner 与连接器选择彼此独立，因此 Marine 或「AI 生成」都复用当前选中的连接器。AI 不再在展开层显示独立“生成”按钮：右侧主按钮在无内容时是禁用的 AI 图标，有源文时变为可请求的 AI 图标，生成中显示转圈，结果就绪后变为纸飞机。没有未决组字时，Return 与这个主按钮共用同一状态：先请求 AI，结果就绪后的新一次轻按才发送下一块，长按仍发送全部。上下轨的角色用图标表示，不再显示“原/答”汉字标记。请求后源文保留在上方轨道；首字返回前会持续展示连接、思考摘要、重试或校验状态与已等待秒数，正文到达后按增量真正流式更新，并由宿主细分为短句、分句、列表项或步骤 block（URL、数字、引文与代码保持完整）。完成后才可手动发送；只有下方所有目标 block 都成功发送后，生成时捕获的源 block 才会一次性消费，部分发送失败不会丢失源文。

「意识流输入」是另一个独立的内置缓冲插件。缓冲开启且它被选中时，会把无修饰的物理 `a-z` 解释为连续全拼，但不读取、不持久化也不切换用户当前的输入方案；并击、互击、双拼或其他已选配置会原样保留。这些纯小写 raw 字母由焦点绑定的独立 workspace 持有，不进入 Rime preedit、候选、`BufferModel` 或宿主文本框；Space 与标点由该轨消费但不写入 raw，数字 `1`–`3` 只用于切换候选。输入停顿 220 ms 后自动全局猜测，持续输入最长等待 800 ms。每个字母只重置 220 ms 停顿计时，800 ms 从本轮连续输入开始计算、不会被后续按键顺延。它不跟随普通「AI 生成」的连接器选择，而是固定走用户已保存的 OpenAI 兼容配置（当前模型为 `deepseek-v4-flash`）。

意识流采用最多两路的有界 make-before-break：旧请求只负责维持可见 baseline，新请求首个合法 partial 或终态到达后才取消旧路；如果两路都在等待，更多按键只更新一个 latest-only pending，不会继续扩散请求。每一轮都重新发送当时的完整纯字母 raw 拼音，不把新增字母分区推断、把旧结果放进 prompt，或与旧结果拼接。本地音节提示器会给同一次全局请求附上最多三条 lossless 边界读法，保留 English 与错键片段，但它不生成中文，也不分区调用模型。新的流式 partial 按公共前缀和稳定候选槽位原位续写；开头分歧不足四个可读字符时先保留旧 baseline，达到阈值后再替换，避免整条清空闪烁。final 到达后精确覆盖本轮槽位，任何旧尾都不能混进可投递正文。所选答案被细分后，首次逐块投递会锁定该候选；继续输入字母则丢弃未发送尾部并建立全新的 raw，已经发送的前缀不会在下一轮复活。waiting/running 阶段不显示选中态，也不授予投递资格；墓碑后的迟到回调会被作废。

上轨显示连续拼音，下轨显示 1–3 个按可能性排序的完整、互斥猜测：输入明确时只给一个，确有两歧或模糊时才增加备选，不能把同一句话拆成多个候选。按数字 `1`–`3` 切换选中项；选中的完整版本再由宿主细分为短句/英文短词组，首次 Return 只强制完成猜测并吞掉同一次物理按键，结果就绪后的下一次 Return 或纸飞机逐块投递该版本，长按 Return 才投递其全部剩余块。换字段、切插件、关闭缓冲或进入 secure input 会取消任务并清除 raw 与结果。
