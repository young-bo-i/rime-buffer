...
- `AITextPluginWorkspace` 只对应一个 `builtin.ai-text`「AI 生成」缓冲插件，对当前 `BufferModel` 全文做显式生成快照；继续打字、切插件、关闭、secure input、刷新或切连接器都会取消/作废旧 generation。首字前显示安全活动摘要和单调等待秒数；正文按 provider delta 真流式进入，并细分为短句/分句/列表项/步骤 block，URL、数字、引文与代码不被切断。block index 原位更新并保持 UUID；只有完成且仍匹配源快照的 target blocks 可投递。目标块全部成功发送后才一次性消费捕获的源 block，部分失败保留剩余目标与全部源文。
