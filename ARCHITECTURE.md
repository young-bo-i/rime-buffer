...
>
> **2026-07-17 当前缓冲 UI/按键覆盖**：工作台折叠为 44pt 单行细条，全部功能从细条向上展开到总高 78pt，展开前后底边与候选锚点不动；缓冲模式复用常规 `CandidateWindow`，默认显示在细条下方。普通/Shift+Return 与 Backspace 在缓冲模式下永不落到宿主文本框：若有未决组字/并击，本次 Return 只收束为块；否则轻按发送下一块，按住约 1.2 秒发送全部，条底部显示按住进度。展开工具层的纸飞机保留为显式发送全部入口。成功发送的 block 立即从 live buffer 消失并进入最近 50 条进程内历史；失败和未发送 block 保留。本文若有更早的投影/留块描述，以本条和 `SYSTEM-ARCHITECTURE.md` 为准。
...
`Delivery.insert` 仍是唯一上屏咽喉。缓冲发送可由 Return 轻按/长按或展开工具层的纸飞机发起，但都必须交给 `BufferDeliveryCoordinator`。键盘路径固定使用 keyDown 时捕获的 `FocusToken`；协调器在每个块投递前重新校验 token、组字状态和 secure input。焦点变化立即停止，既不会改送新目标，也不会回送旧目标；未发送块保留。输入法自身窗口不是投递目标，也不参与远端镜像。

### 5.10 BufferWindowController + BufferModel（P2）— 状态：✅ 已实现

- 工作台是独立、nonactivating、可拖动并可调整宽度的 `NSPanel`。折叠态固定为 44pt 单行主条，只保留缓冲轨与展开入口；目标状态、发送、清空及其余功能统一向上展开，展开态总高 78pt。切换状态保持底边和候选锚点不动；圆角材质内缩到透明窗口边距，并按 backing scale 在路径内绘制 hairline，避免边缘/圆角被裁成毛边。显隐、frame、展开态、pin 与候选锚点持久化，多屏变化时恢复到可见区域。普通关闭会收束组字、保存并擦净已打开的块编辑器、暂停捕获并保留模型内容；secure/隐私/锁屏保护则不保存编辑中明文。清空是独立动作且支持一次撤销。
- Rime commit 只在捕获开启时进入 `BufferModel`；preedit 永不存入模型。成功调用 `Delivery.insert` 后，该 block 立即从 live buffer 消失并写入最近 50 条进程内历史；失败 block 和未发送后缀保留。历史菜单允许用户显式恢复，并按发送前的原相对顺序重新插入。
- 显式单块编辑保留 id、origin、createdAt，并让修改后的块重新待发送。编辑器使用独立 key window；输入法自身所有文本框都绕过缓冲捕获与远端镜像。
- 候选默认使用常规 `CandidateWindow` 固定显示在缓冲条下方，也可继续跟随 caret；工作台自身不含候选或全文预览。常显、移屏、临时遮蔽、secure-input 检测与锁屏隐藏都由 `BufferWindowController` 管理；遮蔽/secure input 会同时关闭并擦净块编辑器的隐藏控件。锁屏、睡眠或会话切出会撤销租约、在 Rime 内收束组字并隐藏窗口，恢复后仍必须等新 activation/event。
...
└── Sources/RimeBuffer/
    ...
    ├── BufferModel.swift          ✅ P2 缓冲模型(live块/成功消费/内存历史/清空撤销)
    ├── BufferDeliveryCoordinator.swift ✅ 精确焦点逐块投递/失败后缀保留
    ├── BufferWindowController.swift ✅ 44pt单行条/向上展开至78pt/编辑/多屏/隐私
...
.build/release/RimeBuffer buffer-smoke # 成功消费/失败保留/50条历史/隐私丢弃/编辑/清空撤销
