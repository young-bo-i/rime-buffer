...
│    ├─ CandidateWindow      候选状态机：caret NSPanel / 工作台投影             │
...
- **键路由规则**（顺序敏感）：
  1. `recognizedEvents` = keyDown | keyUp | flagsChanged。`keyUp` 是缓冲区 Enter 点按/长按手势的必要事件。
...
### 5.5 CandidateWindow — 状态：✅ 候选状态机与双呈现已实现
...
- 缓冲模式默认不显示跟随 caret 的 panel，而是发布带 `FocusToken` 的 `CandidateProjection` 到独立工作台；用户可在设置中切回 caret。两种呈现共用矩阵翻页、单字选择与提交状态，过期点击无效。
...
为当前 controller/client 建立单调 `FocusToken` 租约，并同时校验租约 client 与 `controller.client()` 的对象身份、bundle id、前台应用 PID 与事件顺序。迟到的 deactivate、command、hide、候选点击、Enter 长按或和弦 timer 只操作自己的 token；缓冲投递不存在 recent/last client 回退。NSWorkspace、锁屏/会话与输入源通知负责缺失生命周期回调时的最终撤销。同 proxy 切字段（跨 controller 也算）或旧 marked range 消失时建立新 epoch；旧 session 只在缓冲开启时回收到工作台，否则丢弃，绝不经已经指向新字段的 proxy 提交。弱 client 自然释放时也先清理仍存活 controller 的 chord/session，再移除租约。

### 5.9 Delivery — 状态：✅ 已实现

`Delivery.insert` 仍是唯一上屏咽喉。`BufferDeliveryCoordinator` 只接受当前精确焦点租约，在每个块投递前重新校验 token、组字状态和 secure input。焦点变化立即停止，未发送块保留；输入法自身窗口不是投递目标，也不参与远端镜像。

### 5.10 BufferWindowController + BufferModel（P2）— 状态：✅ 已实现

- 工作台是独立、nonactivating、可拖动和缩放的 `NSPanel`。显隐、frame、pin 与候选呈现位置持久化，多屏变化时恢复到可见区域。普通关闭会收束组字、保存并擦净已打开的块编辑器、暂停捕获并保留模型内容；secure/隐私/锁屏保护则不保存编辑中明文。清空是独立动作且支持一次撤销。
- Rime commit 只在捕获开启时进入 `BufferModel`；preedit 永不存入模型。宿主输入框没有 ACK，所以成功调用后块仍保留并标记 `lastSentAt`。最近 50 条发送快照只在内存中，可从工作台下拉列表选择并恢复为待发送。
- 显式单块编辑保留 id、origin、createdAt，并让修改后的块重新待发送。编辑器使用独立 key window；输入法自身所有文本框都绕过缓冲捕获与远端镜像。
- 候选默认通过 token 化投影固定显示在工作台，也可继续跟随 caret。常显、移屏、临时遮蔽、secure-input 检测与锁屏隐藏都由 `BufferWindowController` 管理；遮蔽/secure input 会同时关闭并擦净块编辑器的隐藏控件。锁屏、睡眠或会话切出会撤销租约、在 Rime 内收束组字并隐藏窗口，恢复后仍必须等新 activation/event。
...
└── Sources/RimeBuffer/
    ...
    ├── CandidateWindow.swift  ✅ 候选状态机+caret面板+token化工作台投影
    ...
    ├── BufferModel.swift          ✅ P2 缓冲模型(块/发送标记/内存历史/清空撤销)
    ├── BufferDeliveryCoordinator.swift ✅ 精确焦点逐块投递
    ├── BufferWindowController.swift ✅ 独立常驻工作台/编辑/多屏/隐私
...
.build/release/RimeBuffer buffer-smoke # 保留发送/50条历史/隐私丢弃/编辑/清空撤销
...
闸口见 §5.10–§5.12。P2 自动验收已覆盖模型保留/历史恢复/编辑元数据、迟到 focus epoch 与多屏 frame；真机还需覆盖 Safari/Electron/微信跨文本框切换、常显/全屏、关闭保留、编辑防回灌以及 secure-input 遮蔽。P4 核心验收：连用一周零"打不出字"事件；改 schema 后进程内重部署生效；password 框优雅直通。
