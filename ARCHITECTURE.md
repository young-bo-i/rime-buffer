...
- `CodexCLITextProvider` 通过双向 stdio JSON-RPC 运行一次性 Codex app-server，消费 `item/agentMessage/delta`；它使用 `~/Library/RimeBuffer/ai/codex-home` 中独立、可持续刷新的 ChatGPT 登录，不读取 `~/.codex`。设置页用独立 account/login app-server 流程打开 HTTPS 授权页，按 loginId 绑定完成通知，再以 account/read 验证 ChatGPT 账户；取消、超时、进程退出与迟到回调均 fail-closed。生成前还要求 `mcpServerStatus/list` 为空。`ClaudeCodeCLITextProvider` 使用 `stream-json` partial。两者均用 `Process`、固定 argv/stdin 和 0700 临时工作目录，不经 shell，不把正文放入进程参数或日志，并限制时间与输出大小、关闭工具及会话持久化；ambient API key 被剔除，未知 CLI 版本 fail-closed。“本地 CLI”只表示进程在本机启动，**不代表本地推理**。
- `OpenAICompatibleTextProvider` 调用 `POST {baseURL}/chat/completions` 并要求 `text/event-stream`，消费 SSE delta 和 `[DONE]`；2xx 非 SSE 响应 fail-closed。Base URL、model 与 API key 在“连接器 › AI 模型”管理；远程端点必须 HTTPS，HTTP 仅允许精确 loopback。配置与密钥存于 `~/Library/RimeBuffer/ai/openai-compatible.json` 的 0600 文件，不进 UserDefaults 或日志。
