...
>
> **2026-07-18 当前缓冲 UI/按键覆盖**：工作台折叠为 44pt 单行细条，主条固定为拖拽图标、展开箭头、缓冲块轨和右侧发送按钮；向上展开到总高 78pt 后只显示无去向信息的状态文字、编辑、缓冲 `NSSwitch` 和关闭。展开前后底边与候选锚点不动；缓冲模式复用常规 `CandidateWindow`，默认显示在细条下方。普通/Shift+Return 与 Backspace 在缓冲模式下永不落到宿主文本框：若有未决组字/并击，本次 Return 只收束为块；否则轻按发送下一块，按住约 1.2 秒发送全部，条底部显示按住进度。成功发送的 block 立即从 live buffer 消失且不保留发送历史；失败和未发送 block 保留。本文若有更早的投影/留块描述，以本条和 `SYSTEM-ARCHITECTURE.md` 为准。
...
- 工作台是独立、nonactivating、可调整宽度的 `NSPanel`；只有 22pt 拖拽图标能移动窗口，背景和其他控件不能拖动。折叠态固定为 44pt 单行主条，顺序为拖拽、展开、缓冲轨、发送；展开态总高 78pt，只含状态、编辑、缓冲 `NSSwitch` 和关闭。切换状态保持底边和候选锚点不动；圆角表面使用日/夜固定 palette（不让透明 HUD 材质采样改变对比度），内缩到透明窗口边距，并按 backing scale 在路径内绘制 hairline。显隐、frame、展开态、pin 与候选锚点持久化，多屏变化时恢复到可见区域。普通关闭会收束组字、保存并擦净已打开的块编辑器、暂停捕获、结束 transient 加载/错误状态并保留已有块；secure/锁屏保护则不保存编辑中明文。工作台不再提供手动遮蔽、历史恢复或清空撤销。
...
- 显式单块编辑保留 id、origin、createdAt，并让修改后的块重新待发送。编辑器使用独立 key window，跟随日/夜外观并在重开时进入当前 Space/指针屏幕；空缓冲点击编辑会给出明确状态提示。输入法自身所有文本框都绕过缓冲捕获与远端镜像。
- 候选默认使用常规 `CandidateWindow` 固定显示在缓冲条下方，也可继续跟随 caret；工作台自身不含候选或全文预览。常显与移屏入口保留在设置/输入法菜单；secure-input 检测与锁屏隐藏由 `BufferWindowController` 管理，secure input 会同时关闭并擦净块编辑器的隐藏控件。锁屏、睡眠或会话切出会撤销租约、在 Rime 内收束组字并隐藏窗口，恢复后仍必须等新 activation/event。
...
.build/release/RimeBuffer buffer-smoke # 成功消费/未发顺序/隐私丢弃/编辑/暂停保留
...
闸口见 §5.10–§5.12。P2 自动验收已覆盖成功消费且不留历史、未发顺序、编辑元数据、迟到 focus epoch 与多屏 frame；真机还需覆盖 Safari/Electron/微信跨文本框切换、常显/全屏、关闭保留、编辑防回灌以及 secure-input 遮蔽。P4 核心验收：连用一周零"打不出字"事件；改 schema 后进程内重部署生效；password 框优雅直通。
