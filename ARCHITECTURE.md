...
>
> **2026-07-22 当前缓冲 UI/按键覆盖**：工作台折叠为 44pt 单行细条，主条固定为拖拽图标、展开箭头、缓冲块轨和右侧发送按钮；向上展开到总高 78pt 后依次显示无去向信息的状态文字、插件动作、刷新/重置和关闭。刷新/重置保留缓冲正文，只取消过时插件任务、重新检测上下文或重启当前翻译。工作台不再提供块编辑器或面板内缓冲开关，缓冲启停仍由设置/输入法菜单管理。展开前后底边与候选锚点不动；缓冲模式复用常规 `CandidateWindow`，默认显示在细条下方。普通/Shift+Return 与 Backspace 在缓冲模式下永不落到宿主文本框：若有未决组字/并击，本次 Return 只收束为块；否则 keyDown 定点重建 IME guard，轻按发送下一块，按住约 1.2 秒发送全部，并显示底部进度。右侧纸飞机每次只发送下一块。成功发送的 block 立即从 live buffer 消失且不保留发送历史；失败和未发送 block 保留。本文若有更早的投影/留块描述，以本条和 `SYSTEM-ARCHITECTURE.md` 为准。
>
...
>
> **2026-07-22 AI 插件/连接器覆盖**：「AI 生成」是唯一内置 `.bufferAction`；Codex CLI、Claude Code CLI 与 OpenAI 兼容 API 是“连接器 › AI 模型”中独立单选的三个模型源。AI 不再在展开层提供独立生成按钮；主条右侧同一控件按 disabled AI 图标 → 可请求 AI 图标 → 转圈 → 纸飞机变化，Return 与它共用这个状态机。生成把 source 冻结在上轨、以稳定 block 更新下轨，只有目标块全部成功投递后才消费对应源块；两轨角色用图标而非“原/答”文字。Marine 继续由 Action Plugin `preparePath` 冻结上下文，插件 owner 与连接器选择互不改写。

> **2026-07-21 意识流输入覆盖**：新增内置 `builtin.stream-input`。仅在缓冲持久开启、插件为唯一 owner、配置为全拼串击、secure input 关闭且外部 `FocusToken` 精确存活时，`RimeBufferController` 才会在进入 Rime 前截获无修饰 `a-z` 与其拥有的分隔键。raw 拼音及目标块只存在 `StreamInputWorkspace`，不写入 Rime/CompositionSession/BufferModel；焦点、插件、配置、缓冲或保护状态失效即取消并清空。字母停顿/最长等待自动触发整段推断：每键只重置 220 ms debounce，800 ms burst 上限不重置；Space 规范化为一个短句边界并立即进入同一有界调度，继续输入后仍会在停手时刷新最新完整 raw，首部/连续 Space 不创建重复工作；其他标点继续消费并忽略。调度最多允许旧视觉生产者与新 challenger 两路短暂重叠；新路首个合法 snapshot/终态到达后才取消旧路，两路占满时只覆盖一个 latest-only pending，绝不形成请求风暴。旧请求只可产出不可提交的临时 partial/final，不能进入下一轮 prompt、获得投递租约或保留选中态，墓碑后的迟到回调全部作废。投递前再次复核同一焦点和 generation。该 workspace 不跟随普通 AI 连接器单选，而是专门使用 `~/Library/RimeBuffer/ai/openai-compatible.json` 中用户保存的 OpenAI 兼容配置（当前模型 `deepseek-v4-flash`），并对意识流请求显式关闭 thinking、要求 JSON object、限制 1024 output tokens。每轮都向模型提交当前完整 raw 拼音及最多三条本地 lossless 音节边界提示并全局重算；本地提示保留 Space 短句边界，不生成中文、不分区调用模型，超过 512 bytes 时直接省略提示但仍发送完整 raw。旧结果只作不可发送的视觉基线，新 partial 依据公共前缀和稳定候选槽位原位续写，短分歧达到可读长度后再替换，final 精确覆盖，禁止分区拼接或把旧尾带入交付。模型返回 1–3 个按可能性排序的完整、互斥猜测，只在真实歧义时给多个；数字 `1`–`3` 切换选中项，Return 或纸飞机只投递所选版本。该 0600 配置位于 app bundle 外，开发重装、pkg 覆盖安装与应用内更新均须保留，升级后无需重复填写。
...
- 工作台是独立、nonactivating、可调整宽度的 `NSPanel`；只有 22pt 拖拽图标能移动窗口，背景和其他控件不能拖动。普通缓冲折叠/展开严格保持 44/78pt；苹果翻译与 AI 生成插件打开时主条改为上 source、下 target 两条独立横向滚动轨，折叠/展开为 78/112pt；拖拽与展开控件对齐上方源文行，右侧主控件对齐下方目标行。AI 状态下该主控件在请求/转圈/纸飞机间原位切换，展开区不再放第二个生成按钮。展开区固定为状态、缓冲插件选择器及其他插件的当前动作、刷新/重置与关闭。选择器直接改写唯一 owner，刷新/重置保留缓冲正文并只重置当前插件运行状态。切换插件和展开状态始终固定底边与候选锚点。圆角表面使用日/夜固定 palette，内缩到透明窗口边距，并按 backing scale 在路径内绘制 hairline。显隐、frame、展开态、pin 与候选锚点持久化，多屏变化时恢复到可见区域。普通关闭会收束组字、暂停捕获、结束 transient 加载/错误状态并保留已有块。`Command+Shift+B` 通过全局 Carbon hot key 调用 `toggleVisibility()`；工作台不提供块编辑器、面板内缓冲开关、手动遮蔽、历史恢复或清空撤销。
...
- `AITextPluginWorkspace` 只对应一个 `builtin.ai-text`「AI 生成」缓冲插件，对当前 `BufferModel` 全文做显式生成快照；继续打字、切插件、关闭、secure input、刷新或切连接器都会取消/作废旧 generation。`DerivedManualGenerationPrimaryAction` 统一决定窗口主按钮与 Return：无源文为 disabled，有源文为 requestGeneration，running 为转圈，ready 为 deliver；请求和等待状态在 keyDown 就拥有整次物理 Return，防止同步/快速结果被同一次 keyUp 立即发送。请求前还在动作边界同步重查 secure input，命中时只吞键并立即保护全部派生 workspace，不依赖窗口轮询。首字前显示安全活动摘要和单调等待秒数；正文按 provider delta 真流式进入，并细分为短句/分句、列表项或步骤 block，URL、数字、引文与代码不被切断。block index 原位更新并保持 UUID；只有完成且仍匹配源快照的 target blocks 可投递。目标块全部成功发送后才一次性消费捕获的源 block，部分失败保留剩余目标与全部源文。
...
- `StreamInputWorkspace` 是第三种派生双轨 source，不读取 `BufferModel`：它把 raw 全拼绑定到创建它的精确外部焦点，按 220/800 ms debounce/max-wait 自动请求专用的 `OpenAICompatibleTextProvider`，直接读取用户已保存的 OpenAI 兼容配置而不跟随普通 AI 连接器单选。连续字母中每个按键只重置 220 ms debounce；从 burst 首键建立的 800 ms max-wait 不重置，保证一直输入也会刷新。Space 被规范化为一个短句硬边界并立即触发当前完整 raw；首部/重复 Space 不改变 revision，Space 后续输入仍通过尾部 debounce 刷新，其他标点由 workspace 消费但忽略。workspace 最多允许两个 provider 请求短暂重叠：旧路仅维持视觉连续，新 challenger 的首个合法非空 snapshot 或终态到达后才取消/墓碑旧路；两路都占用时只保留一个 latest-only pending，槽位打开后冻结当时最新完整 raw，不能启动第三路 fan-out。
- 每次意识流请求的 source 都是触发边界时的完整 raw 拼音，模型必须从全局重新解释，不能只处理新增后缀、把旧显示或旧响应放进 prompt，或把输入分段拼接。本地 `StreamInputPinyinHints` 只用 tone-free 音节表生成最多三条全覆盖、lossless 边界提示；Space 是不可跨越的短句边界，无法识别的 English/错键片段原样保留，超过 512 bytes 时省略提示但仍照常发送完整 raw。它不生成中文，也不改变全局单次请求边界。waiting/running 时临时结果不显示选中态，也不能建立 result/delivery lease。流式 partial 以候选 index 的稳定槽位复用身份，按它与 baseline/当前文本的公共前缀原位续写；开头分歧的新 partial 少于四个可读字符时先保留旧 baseline，达到阈值后原位替换。final 是当前完整 raw 请求的唯一权威值，必须逐槽精确覆盖 partial；旧 baseline 的剩余尾部不得进入 ready block、投递快照或正文。模型产出 1–3 个按可能性排序的完整、互斥猜测，明确输入只保留一个，真实歧义才增加备选；每个版本都必须能独立上屏，不得把一句正文拆成多个候选。数字 `1`–`3` 只切换选中项且不污染正文；仅所选 block 能进入投递快照。首次 Return 在未 ready 时只强制生成并拥有该物理按键，ready 后的新 Return 或纸飞机只投递选中版本。所有异步结果还必须与 raw、焦点和 delivery generation 一致。
